                        ...
                        <tbody>
                            {props.post.map(post => (
                                ...
                                >
                                    ...
                                    <td className="border-t">
                                        {post.status == 1 ? <h5 className="flex bg-green-100 text-green-600 items-center px-6 py-4 focus:text-indigo-700 focus:outline-none">
                                            Approved
                                        </h5> : post.status == 2 ? <h5 className="flex bg-red-100 text-red-600 items-center px-6 py-4 focus:text-indigo-700 focus:outline-none">
                                            Denied
                                        </h5> : <h5 className="flex bg-yellow-100 text-yellow-600 items-center px-6 py-4 focus:text-indigo-700 focus:outline-none">
                                            Awaiting Approval
                                        </h5>}
                                    </td>
                                    <td className="mt-2 border-t">
                                        ...
                                        {/* <Link as='button' method='post' href={`/dashboard/post/edit/${post.id}?status=true`} className={post.status == 1 || post.status == 2 ? 'hidden' : `bg-green-500 text-white inline items-center px-3 py-2 mx-2 focus:text-indigo-700 focus:outline-none`}>
                                        ...
                                        <button onClick={() => { handleAccept(post.id); }} className={post.status == 1 || post.status == 2 ? 'hidden' : `bg-green-500 text-white inline items-center px-3 py-2 mx-2 focus:text-indigo-700 focus:outline-none`}>
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 inline mx-1 my-auto" viewBox="0 0 20 20" fill="currentColor">
                                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                                            </svg>
                                        </button>
                                        <button onClick={() => { handleDelete(post.id); }} className={post.status == 1 || post.status == 2 ? 'hidden' : `bg-red-500 text-white inline items-center px-3 py-2 mx-2 focus:text-indigo-700 focus:outline-none`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 inline mx-1 my-auto" viewBox="0 0 20 20" fill="currentColor">
